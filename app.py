...
for feature_path in Path(constants.FEATURED).glob("*.npy"):
    features.append(numpy.load(feature_path))
...
def index():
    if request.method == 'POST':
        ...
        dists = numpy.linalg.norm(features - query, axis=1)
        ids = numpy.argsort(dists)[:30]  # Top 30 results
...
def train():
    ...
    if request_data:
        ...
        response = make_response(
            jsonify({
                "error": "This endpoint requires request paramter named: images only, and dosen't accept empty body "
                         "requests"}),
            400,
        )
        response.headers["Content-Type"] = "application/json"
